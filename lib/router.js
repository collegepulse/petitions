var seoData = {
  ...
  postsResponses: {
    title: "Responses",
    description: "View petitions that have recieved responses from RIT Student Government and Administration."
  },
  postPage: {
    title: "View Petition",
...
Router.configure({
  ...
  onRun: function() {
    Session.set('page', this.route.name);
    Deps.nonreactive(function () {
      setSEO(seoData[Session.get('page')]);
      GAnalytics.pageview();
    });
  },
  waitOn: function() {
  ...
  if (Meteor.isClient) {
    SEO.set({
      ...
      og: {
        'title': data.title,
        'description': data.description,
        'image': window.location.origin + '/logo_200x200.png'
      }
    });
  }
};
