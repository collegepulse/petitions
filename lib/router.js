...
PetitionsListController = RouteController.extend({
  ...
  onBeforeAction: function() {
    Meteor.subscribe('petitions', Session.get('petitionsLimit'), Session.get('petitionOrder'), Session.get('activeTag'));
    infiniteScroll();
    this.next();
  },
  data: petitionSort
...
PetitionsWithResponsesController = RouteController.extend({
  ...
  onBeforeAction: function () {
    Meteor.subscribe('petitionsWithResponses', Session.get('petitionsLimit'), Session.get('petitionOrder'), Session.get('activeTag'));
    infiniteScroll();
    this.next();
  },
  data: petitionSort
...
PetitionsInProgressController = RouteController.extend({
  ...
  onBeforeAction: function () {
    Meteor.subscribe('petitionsInProgress', Session.get('petitionsLimit'), Session.get('petitionOrder'), Session.get('activeTag'));
    infiniteScroll();
    this.next();
  },
  data: petitionSort
...
Router.map(function() {
  ...
  this.route('search', {
    ...
    template: 'search',
    waitOn: function() {
      return [Meteor.subscribe('petitions')]
    }
  });
